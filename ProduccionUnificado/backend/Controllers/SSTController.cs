...
public class SSTController : ControllerBase
{
    ...
    {
        var gastos = await _context.SST_GastosMensuales
            .Include(g => g.Rubro)
    ...
    public async Task<ActionResult<object>> GetGastosResumen([FromQuery] int anio, [FromQuery] int mes)
    {
        ...
        // Get presupuestos for the month
        var presupuestos = await _context.SST_PresupuestosMensuales
            .Include(p => p.TipoServicio)
            .Where(p => p.Anio == anio && p.Mes == mes)
            .ToListAsync();

        // Group gastos by TipoServicio
        var resumenPorServicio = presupuestos.Select(p => new
        {
            TipoServicioId = p.TipoServicioId,
            TipoServicioNombre = p.TipoServicio!.Nombre,
            Presupuesto = p.Presupuesto,
            Gastado = gastos.Where(g => g.TipoServicioId == p.TipoServicioId).Sum(g => g.Precio),
            Restante = p.Presupuesto - gastos.Where(g => g.TipoServicioId == p.TipoServicioId).Sum(g => g.Precio)
        }).ToList();
        ...
        return Ok(new
        {
            ...
            DetallesPorServicio = resumenPorServicio
        });
    }
