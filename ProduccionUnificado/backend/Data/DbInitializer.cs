...
public static class DbInitializer
{
    ...
    private static void CreateTables(AppDbContext context)
    {
        ...
        try 
        {
            context.Database.ExecuteSqlRaw(@"
                ...
                FROM (VALUES
                    ...
                    (3, 'Guillotina 2A polar132', 30000, 0.25, 2, 1250, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (4, 'Guillotina 2B org- Perfecta 107', 30000, 0.25, 2, 1250, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (5, '3 Sord Z', 15000, 0.25, 5, 1250, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (6, '4 Sord Z', 15000, 0.25, 5, 2000, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (7, '5 Sord Z', 15000, 0.25, 5, 1250, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (8, '6 SpeedMaster', 15000, 0.25, 5, 3000, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (9, '7 SpeedMaster', 22500, 0.25, 5, 3000, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (10, '8A Troqueladora de Papel', 7500, 0.25, 10, 1000, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (11, '8B Troqueladora de Papel', 7500, 0.25, 10, 1000, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (12, '8C Estampadora', 6000, 0.25, 12, 1500, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (13, '9 Troqueladora Rollo', 15000, 0.25, 5, 1250, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (14, '10A Colaminadora Carton', 7500, 0.07, 10, 500, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (15, '10B Colaminadora Carton', 6000, 0.03, 12, 400, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (16, '11 Laminadora BOPP', 7500, 0.25, 10, 1000, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (103, '16 Barnizadora UV', 7500, 0.25, 10, 1250, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (18, '13A Corrugadora FLTE', 2250, 0.25, 40, 2000, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (19, '13b Corrugadora FLTB', 2250, 0.25, 35, 1250, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (20, '14 Pegadora de Cajas', 75000, 0.07, 1, 40000, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
                    (21, '15 Troqueladora Kirby', 1500, 0.25, 40, 1250, 0, 0, 0, 0, 0, CAST(1 AS BOOLEAN)),
        ...
        try
        {
            ...
            Console.WriteLine("[DB DEBUG] About to create Produccion_TiposRecargo...");
            try {
                context.Database.ExecuteSqlRaw(@"
                ...
                ");
                Console.WriteLine("[DB INIT] Produccion_TiposRecargo created.");
            } catch (Exception ex) { Console.WriteLine($"[DB ERROR] TiposRecargo: {ex.Message}"); }
            ...
            {
                var recargos = new List<TiempoProcesos.API.Models.Produccion_TipoRecargo>
                {
                    new() { Nombre = "Recargo Nocturno", Porcentaje = 35, Factor = 1.35m },
                    new() { Nombre = "Recargo Dominical/Festivo", Porcentaje = 75, Factor = 1.75m }
                };
                context.Produccion_TiposRecargo.AddRange(recargos);
                context.SaveChanges();
            }
        ...
        try
        {
            Console.WriteLine("[DB DEBUG] Creating Planeacion tables...");

            // Planeacion_Personal
            context.Database.ExecuteSqlRaw(@"
                CREATE TABLE IF NOT EXISTS ""Planeacion_Personal"" (
                    ""Id"" SERIAL PRIMARY KEY,
                    ""Nombre"" VARCHAR(200) NOT NULL,
            ...
            ");
            Console.WriteLine("[DB INIT] Planeacion_Personal created.");

            // Planeacion_Rubros
            context.Database.ExecuteSqlRaw(@"
                CREATE TABLE IF NOT EXISTS ""Planeacion_Rubros"" (
                    ""Id"" SERIAL PRIMARY KEY,
                    ""Nombre"" VARCHAR(200) NOT NULL,
                    ""Activo"" BOOLEAN NOT NULL DEFAULT TRUE
                );
            ");
            Console.WriteLine("[DB INIT] Planeacion_Rubros created.");

            // Planeacion_Proveedores
            context.Database.ExecuteSqlRaw(@"
                CREATE TABLE IF NOT EXISTS ""Planeacion_Proveedores"" (
                    ""Id"" SERIAL PRIMARY KEY,
                    ""Nombre"" VARCHAR(200) NOT NULL,
                    ""NitCedula"" VARCHAR(50) NOT NULL,
                    ""Telefono"" VARCHAR(50) NULL,
                    ""RubroId"" INTEGER NOT NULL REFERENCES ""Planeacion_Rubros""(""Id""),
                    ""PrecioCotizado"" DECIMAL(18,2) NULL,
                    ""Activo"" BOOLEAN NOT NULL DEFAULT TRUE
                );
            ");
            Console.WriteLine("[DB INIT] Planeacion_Proveedores created.");

            // Planeacion_Cotizaciones
            context.Database.ExecuteSqlRaw(@"
                CREATE TABLE IF NOT EXISTS ""Planeacion_Cotizaciones"" (
                    ""Id"" SERIAL PRIMARY KEY,
                    ""ProveedorId"" INTEGER NOT NULL REFERENCES ""Planeacion_Proveedores""(""Id""),
                    ""RubroId"" INTEGER NOT NULL REFERENCES ""Planeacion_Rubros""(""Id""),
                    ""Anio"" INTEGER NOT NULL,
                    ""Mes"" INTEGER NOT NULL,
                    ""PrecioCotizado"" DECIMAL(18,2) NOT NULL,
            ...
            ");
            Console.WriteLine("[DB INIT] Planeacion_Cotizaciones created.");

            // Planeacion_Gastos
            context.Database.ExecuteSqlRaw(@"
                CREATE TABLE IF NOT EXISTS ""Planeacion_Gastos"" (
                    ""Id"" SERIAL PRIMARY KEY,
                    ""RubroId"" INTEGER NOT NULL REFERENCES ""Planeacion_Rubros""(""Id""),
                    ""ProveedorId"" INTEGER NULL REFERENCES ""Planeacion_Proveedores""(""Id""),
                    ""Anio"" INTEGER NOT NULL,
                    ""Mes"" INTEGER NOT NULL,
                    ...
                    ""CreadoPorId"" INTEGER NULL REFERENCES ""AdminUsuarios""(""Id""),
                    ""PersonalId"" INTEGER NULL REFERENCES ""Planeacion_Personal""(""Id""),
                    ""TipoHoraId"" INTEGER NULL REFERENCES ""Produccion_TiposHora""(""Id""),
                    ""TipoRecargoId"" INTEGER NULL REFERENCES ""Produccion_TiposRecargo""(""Id""),
                    ""CantidadHoras"" DECIMAL(18,2) NULL,
                    ""NumeroOP"" VARCHAR(100) NULL
                );

                -- Ensure columns for overtime/surcharge if they don't exist
                ALTER TABLE ""Planeacion_Gastos"" ADD COLUMN IF NOT EXISTS ""PersonalId"" INTEGER NULL REFERENCES ""Planeacion_Personal""(""Id"");
                ALTER TABLE ""Planeacion_Gastos"" ADD COLUMN IF NOT EXISTS ""TipoHoraId"" INTEGER NULL REFERENCES ""Produccion_TiposHora""(""Id"");
                ALTER TABLE ""Planeacion_Gastos"" ADD COLUMN IF NOT EXISTS ""TipoRecargoId"" INTEGER NULL REFERENCES ""Produccion_TiposRecargo""(""Id"");
                ALTER TABLE ""Planeacion_Gastos"" ADD COLUMN IF NOT EXISTS ""CantidadHoras"" DECIMAL(18,2) NULL;
                ALTER TABLE ""Planeacion_Gastos"" ADD COLUMN IF NOT EXISTS ""NumeroOP"" VARCHAR(100) NULL;
                ALTER TABLE ""Planeacion_Gastos"" ALTER COLUMN ""NumeroFactura"" DROP NOT NULL;
            ");
            Console.WriteLine("[DB INIT] Planeacion_Gastos checked/updated.");

            // Planeacion_PresupuestosMensuales
            context.Database.ExecuteSqlRaw(@"
                CREATE TABLE IF NOT EXISTS ""Planeacion_PresupuestosMensuales"" (
                    ""Id"" SERIAL PRIMARY KEY,
                    ""RubroId"" INTEGER NOT NULL REFERENCES ""Planeacion_Rubros""(""Id""),
                    ""Anio"" INTEGER NOT NULL,
                    ""Mes"" INTEGER NOT NULL,
            ...
            ");
            Console.WriteLine("[DB INIT] Planeacion_PresupuestosMensuales created.");

            // Seed Planeacion Rubros
            if (!context.Planeacion_Rubros.Any())
            {
                var rubros = new List<TiempoProcesos.API.Models.Planeacion_Rubro>
                {
                    new() { Nombre = "Insumos" },
                    new() { Nombre = "Mantenimiento" },
                    new() { Nombre = "Horas Extras" },
                ...
                context.Planeacion_Rubros.AddRange(rubros);
                context.SaveChanges();
                Console.WriteLine("[DB INIT] Planeacion_Rubros seeded.");
            }
        }
        catch (Exception ex) { Console.WriteLine($"[DB ERROR] Planeacion Tables: {ex.Message}"); }
