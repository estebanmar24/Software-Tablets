...
async function main() {
    ...
    try {
        ...
        console.log('Counting duplicates (ignoring time)...');
        const res = await client.query(`
            SELECT COUNT(*) FROM (
                SELECT "Id",
                       ROW_NUMBER() OVER (
                           PARTITION BY "Fecha"::date, "MaquinaId", "UsuarioId", "OrdenProduccion", "Cantidad"
                           ORDER BY "Fecha" DESC -- Prefer the one with more specific time if exists, or just the latest
                       ) as row_num
