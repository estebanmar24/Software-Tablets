...
function GastosTab({ displayName }) {
    ...
    const nombreRubroLower = (rubroSeleccionado?.nombre || '').toLowerCase();
    const isNominaHeORecargoRubro = nombreRubroLower.includes('hora extra') || nombreRubroLower.includes('recargo');
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            const [rubrosData, proveedoresData, gastosData, graficasData, cotData] = await Promise.all([
                disenoApi.getRubros(),
                disenoApi.getProveedores(),
                disenoApi.getGastos(anio, mes),
                disenoApi.getGraficas(anio, mes),
                disenoApi.getCotizaciones(null, anio, mes)
            ]);
            ...
            try {
                const anualData = await disenoApi.getGraficasAnual(anio);
    ...
    const gastoEsHeRecRubro = (g) => {
        const n = (g?.rubroNombre || '').toLowerCase();
    ...
    const handleUpdateEstado = async (gasto) => {
        ...
        const executeUpdate = async () => {
            try {
                ...
                await disenoApi.updateGasto(gasto.id, updatedGasto);
                await loadData();
    ...
    const handleEdit = (gasto) => {
        ...
        setMedioPago(gastoEsHeRecRubro(gasto) ? null : flagsToMedioPago(!!gasto.esSolicitudCredito, !!gasto.esEfectivo));
        setIsLegalizing(false);
        setShowModal(true);
    };
    ...
    const handleSubmit = async () => {
        ...
        } else if (!isNominaHeORecargoRubro) {
            let pb = parseMontoInput(formData.precioBase);
            let pi = parseMontoInput(formData.precioIva);
            if (pb === null) pb = 0;
            if (pi === null) pi = 0;
        ...
        if (!isNominaHeORecargoRubro && !medioPago) {
            showAlert(ALERT_MEDIO_PAGO_TITULO, ALERT_MEDIO_PAGO_MENSAJE);
            return;
        }
        ...
        try {
            ...
            if (!isNominaHeORecargoRubro && formData.esPendiente) {
                if (pbSave === null) pbSave = 0;
                if (piSave === null) piSave = 0;
            }
            const totalPrecio = isNominaHeORecargoRubro
            ...
            const gastoData = {
                ...
                precio: totalPrecio,
                ...(!isNominaHeORecargoRubro && pbSave !== null && piSave !== null ? { precioBase: pbSave, precioIva: piSave } : {}),
                ...
                ...(isNominaHeORecargoRubro ? { esSolicitudCredito: false, esEfectivo: false } : medioPagoToFlags(medioPago)),
            ...
            if (quote) {
                ...
                if (Math.abs(quotePrice - gastoData.precio) > 1 && Platform.OS === 'web') {
                    if (window.confirm(`Precio diferente a cotización (${formatCurrency(quotePrice)}). ¿Actualizar cotización?`)) {
                        await disenoApi.updateCotizacion(quote.id, { ...quote, precioCotizado: gastoData.precio });
                    }
                }
            }
    ...
    const handleDelete = async (id) => {
        ...
        const doDelete = async () => {
            try { await disenoApi.deleteGasto(id); await loadData(); showAlert('Éxito', 'Gasto eliminado'); }
                            ...
                            <View key={gasto.id} style={styles.gastoCard}>
                                <View style={styles.gastoHeader}>
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                        ...
                                        <GastoListaPrecios
                                            gasto={gasto}
                                            singlePriceRow={gastoEsHeRecRubro(gasto)}
                                            formatCurrency={formatCurrency}
                            ...
                            <>
                                ...
                                {isNominaHeORecargoRubro ? (
                                    ...
                                    <>
                                        ...
                                        <TextInput style={styles.input} value={formData.precioBase} onChangeText={(t) => setFormData(p => ({ ...p, precioBase: t }))} keyboardType="decimal-pad" placeholder="0" />
                                        <Text style={[styles.label, { marginTop: 10 }]}>IVA * (puede ser 0)</Text>
                                        <TextInput style={styles.input} value={formData.precioIva} onChangeText={(t) => setFormData(p => ({ ...p, precioIva: t }))} keyboardType="decimal-pad" placeholder="0" />
...
function GraficasTab() {
        ...
        try {
            ...
            if (mesSeleccionado) {
                [dataGraf, dataGastos] = await Promise.all([
                    disenoApi.getGraficas(anio, mesSeleccionado),
                    disenoApi.getGastos(anio, mesSeleccionado)
                ]);
            } else {
                dataGraf = await disenoApi.getGraficasAnual(anio);
                try { dataGastos = await disenoApi.getGastos(anio, 0); } catch (e) { dataGastos = []; }
            }
    ...
    const generateReport = async () => {
        ...
        try {
            ...
            doc.text(`INFORME DISEÑO - ${mesSeleccionado ? MESES[Number(mesSeleccionado) - 1].label.toUpperCase() : 'ANUAL'} ${anio}`, pageWidth / 2, 20, { align: 'center' });
            yPos = 50;
            ...
            const [allRubros] = await Promise.all([disenoApi.getRubros()]);
            const tableRows = [];
            ...
            doc.save(`Informe_Diseno_${anio}_${mesSeleccionado || 'Anual'}.pdf`);
    ...
    const generateCSV = async () => {
        ...
        try {
            ...
            const filename = `Diseno_Gastos_${anio}_${mesSeleccionado || 'Anual'}.csv`;
    ...
    return (
        <View style={styles.contentContainer}>
            <View style={styles.header}>
                <Text style={styles.title}>📊 Análisis de Gastos Diseño</Text>
...
function PresupuestosTab() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            const [rubrosData, presupuestosData] = await Promise.all([
                disenoApi.getRubros(),
                disenoApi.getPresupuestosGrid(anio)
            ]);
            setRubros(rubrosData);
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            await disenoApi.setPresupuestosBulk(presupuestos.filter(p => p.presupuesto > 0));
...
function RubrosTab() {
    ...
    const loadData = async () => { try { setLoading(true); setItems(await disenoApi.getRubros()); } catch (e) { } finally { setLoading(false); } };
    useEffect(() => { loadData(); }, []);
    ...
    const handleSave = async () => {
        ...
        try {
            setSaving(true); const data = { nombre, activo: true }; if (editItem) await disenoApi.updateRubro(editItem.id, data); else await disenoApi.createRubro(data);
    ...
    const handleDelete = async (id) => {
        const doDelete = async () => { try { await disenoApi.deleteRubro(id); loadData(); showAlert('Éxito', 'Rubro eliminado'); } catch { showAlert('Error', 'No se pudo eliminar'); } };
...
function ProveedoresTab() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (editItem) await disenoApi.updateProveedor(editItem.id, data);
            else await disenoApi.createProveedor(data);
    ...
    const handleDelete = async (id) => {
        const doDelete = async () => { try { await disenoApi.deleteProveedor(id); loadData(); showAlert('Éxito', 'Proveedor eliminado'); } catch { showAlert('Error', 'No se pudo eliminar'); } };
...
function CotizacionesTab() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            const [rubrosData, provData, cotData] = await Promise.all([
                disenoApi.getRubros(),
                disenoApi.getProveedores(),
                disenoApi.getCotizaciones(anio, mes)
            ]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            else await disenoApi.createCotizacion(data);
    ...
    const handleDelete = (id) => {
        const doDelete = async () => { try { await disenoApi.deleteCotizacion(id); loadData(); showAlert('Éxito', 'Cotización eliminada'); } catch (e) { showAlert('Error', 'No se pudo eliminar'); } };
            ...
            <ScrollView style={styles.listContainer}>
                {items.length === 0 ? (<View style={styles.emptyState}><Text style={styles.emptyText}>No hay cotizaciones para este periodo</Text></View>) : (
                    items.map(item => (<View key={item.id} style={styles.gastoCard}>
                        ...
                        <Text style={styles.gastoRubro}>Servicio: {item.rubroNombre}</Text>
