...
// Máquinas
export const getMaquinas = () => axiosWrapper<any[]>(`maquinas`);
export const getMaquinasActivas = () => axiosWrapper<any[]>(`maquinas?soloActivas=true`);
export const createMaquina = (data: any) => axiosWrapper<any>(`maquinas`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data)
});
export const updateMaquina = (id: number, data: any) => axiosWrapper<any>(`maquinas/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data)
});
export const deleteMaquina = (id: number) => axiosWrapper<any>(`maquinas/${id}`, {
    method: 'DELETE'
});
...
export const updateUsuario = (id: number, data: any) => axiosWrapper<any>(`usuarios/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data)
});

export const deleteUsuario = (id: number) => axiosWrapper<any>(`usuarios/${id}`, {
    method: 'DELETE'
});

// Producción
export const saveProduccion = (data: any) => axiosWrapper<any>(`produccion`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data)
});
...
export const getResumen = (mes: number, anio: number) =>
    axiosWrapper<any>(`produccion/resumen?mes=${mes}&anio=${anio}`);

export const getProduccionDetalles = (mes: number, anio: number, maquinaId: number, usuarioId: number) =>
    axiosWrapper<any[]>(`produccion/detalles?mes=${mes}&anio=${anio}&maquinaId=${maquinaId}&usuarioId=${usuarioId}&_t=${Date.now()}`);

export const getOperariosConDatos = (mes: number, anio: number) =>
    axiosWrapper<any[]>(`produccion/operarios-con-datos?mes=${mes}&anio=${anio}&_t=${Date.now()}`);

export const getMaquinasConDatos = (mes: number, anio: number) =>
    axiosWrapper<any[]>(`produccion/maquinas-con-datos?mes=${mes}&anio=${anio}&_t=${Date.now()}`);

export const getProduccionPorMaquina = (mes: number, anio: number, maquinaId: number) =>
    axiosWrapper<any[]>(`produccion/detalles-maquina?mes=${mes}&anio=${anio}&maquinaId=${maquinaId}&_t=${Date.now()}`);

export const borrarProduccion = (mes: number, anio: number, usuarioId?: number, maquinaId?: number) => {
    let url = `produccion/borrar?mes=${mes}&anio=${anio}`;
    if (usuarioId) url += `&usuarioId=${usuarioId}`;
    if (maquinaId) url += `&maquinaId=${maquinaId}`;
    return axiosWrapper<any>(url, { method: 'DELETE' });
};

export const getPeriodosDisponibles = () => axiosWrapper<any[]>(`produccion/periodos-disponibles`);
